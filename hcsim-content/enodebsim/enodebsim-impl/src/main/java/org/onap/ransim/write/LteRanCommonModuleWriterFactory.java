 ...
 */
public final class LteRanCommonModuleWriterFactory implements WriterFactory {

    private static final InstanceIdentifier<LteRan> ROOT_CONTAINER_ID = InstanceIdentifier.create(LteRan.class);
    private static final Logger LOG = LoggerFactory.getLogger(LteRanCommonModuleWriterFactory.class);
    ...
    public void init(@Nonnull final ModifiableWriterRegistryBuilder registry) {
        LOG.info("RANSIM LteRanCommonModuleWriterFactory init called");

        //adds writer for child node
        //no need to add writers for empty nodes
        registry.add(new GenericWriter<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(CellConfig.class).child(Lte.class).child(LteRan.class).child(LteRanCommon.class), new LteRanCommonCustomizer(crudService)));
    }
}
