 ...
 */
public final class CellConfigModuleWriterFactory implements WriterFactory {

    private static final Logger LOG = LoggerFactory.getLogger(CellConfigModuleWriterFactory.class);
    ...
    public void init(@Nonnull final ModifiableWriterRegistryBuilder registry) {
        LOG.info("RANSIM CellConfigModuleWriterFactory init called");

        //adds writer for child node
        //no need to add writers for empty nodes
        registry.add(new GenericWriter<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(CellConfig.class), new CellConfigCustomizer(crudService)));
    }
}
