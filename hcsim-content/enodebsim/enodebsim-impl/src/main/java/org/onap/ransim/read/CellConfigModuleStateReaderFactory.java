...
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev181127.radio.access.fap.service.CellConfigBuilder;
 ...
 */
public final class CellConfigModuleStateReaderFactory implements ReaderFactory {

    private static final Logger LOG = LoggerFactory.getLogger(CellConfigModuleStateReaderFactory.class);
    ...
    public static final InstanceIdentifier<CellConfig> LRCROOT_STATE_CONTAINER_ID =
            InstanceIdentifier.create(CellConfig.class);
     ...
     */
    @Inject
    @Named(CC_SERVICE_NAME)
    private CrudService<CellConfig> crudService;

    @Override
    public void init(@Nonnull final ModifiableReaderRegistryBuilder registry) {
        LOG.info("RANSIM CellConfigModuleStateReaderFactory init called");

        // register reader that only delegate read's to its children
        registry.addStructuralReader(CCROOT_STATE_CONTAINER_ID, CellConfigBuilder.class);
        ...
        registry.add(
                new GenericReader<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(CellConfig.class),
                        new CellConfigCustomizer(crudService)));
