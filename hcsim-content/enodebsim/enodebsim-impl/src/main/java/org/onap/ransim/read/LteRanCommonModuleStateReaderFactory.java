...
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev181127.radio.access.fap.service.cell.config.lte.lte.ran.LteRanCommonBuilder;
 ...
 */
public final class LteRanCommonModuleStateReaderFactory implements ReaderFactory {

    private static final Logger LOG = LoggerFactory.getLogger(LteRanCommonModuleStateReaderFactory.class);
    ...
    public static final InstanceIdentifier<LteRanCommon> LRCROOT_STATE_CONTAINER_ID =
            InstanceIdentifier.create(LteRanCommon.class);
     ...
     */
    @Inject
    @Named(LRC_SERVICE_NAME)
    private CrudService<LteRanCommon> crudService;

    @Override
    public void init(@Nonnull final ModifiableReaderRegistryBuilder registry) {
        LOG.info("RANSIM LteRanCommonModuleStateReaderFactory init called");

        // register reader that only delegate read's to its children
        registry.addStructuralReader(LRCROOT_STATE_CONTAINER_ID, LteRanCommonBuilder.class);
        ...
        registry.add(
                new GenericReader<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(CellConfig.class).child(Lte.class).child(LteRan.class).child(LteRanCommon.class),
                        new LteRanCommonCustomizer(crudService)));
