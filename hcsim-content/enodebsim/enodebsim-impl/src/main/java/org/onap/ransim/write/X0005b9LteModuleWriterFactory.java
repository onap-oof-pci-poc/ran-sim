 ...
 */
public final class X0005b9LteModuleWriterFactory implements WriterFactory {

    private static final Logger LOG = LoggerFactory.getLogger(X0005b9LteModuleWriterFactory.class);
    ...
    public void init(@Nonnull final ModifiableWriterRegistryBuilder registry) {
        LOG.info("RANSIM X0005b9LteModuleWriterFactory init called");

        //adds writer for child node
        //no need to add writers for empty nodes
        registry.add(new GenericWriter<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(X0005b9Lte.class), new X0005b9LteCustomizer(crudService)));
    }
}
