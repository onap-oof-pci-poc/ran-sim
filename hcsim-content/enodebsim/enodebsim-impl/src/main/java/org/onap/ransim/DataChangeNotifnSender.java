...
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.EditOperationType;
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.NetconfConfigChange;
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.NetconfConfigChangeBuilder;
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.RadioAccess;
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.netconf.config.change.Edit;
import org.opendaylight.yang.gen.v1.org.onap.ccsdk.features.sdnr.northbound.oofpcipoc.rev190308.netconf.config.change.EditBuilder;
...
public class DataChangeNotifnSender implements ManagedNotificationProducer {
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(DataChangeNotifnSender.class);
    ...
    public static void sendNotification(UpdateCell updCell) {
        LOG.info("RANSIM DataChangeNotifnSender sendNotification called ****** {}"
                , updCell.toString());
        ...
        for (int i = 0; i < updCell.getOneCell().getNeighborList().size(); i++) {
            ...
            InstanceIdentifier<FapService> identifier = InstanceIdentifier
            .builder(RadioAccess.class)
            .child(FapService.class, new FapServiceKey(nbr.getNodeId()))
            .build();
            edit.setTarget(identifier);
            editList.add(edit.build());
        }
        final NetconfConfigChange notification = new NetconfConfigChangeBuilder()
        .setDatastore(null).setEdit(editList).build();
...
/*
                    ...
                    nbrs.add(new Neighbor("jio", "1", 1, "ncserver1001",
                            "ncserver1001", false));
                    UpdateCell updCell = new UpdateCell("1", "127.0.0.1",
                            "50000",
