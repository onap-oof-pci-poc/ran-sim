 ...
 */
public final class LteRanRfModuleWriterFactory implements WriterFactory {

    private static final Logger LOG = LoggerFactory.getLogger(LteRanRfModuleWriterFactory.class);
    ...
    public void init(@Nonnull final ModifiableWriterRegistryBuilder registry) {
        LOG.info("RANSIM LteRanRfModuleWriterFactory init called");

        //adds writer for child node
        //no need to add writers for empty nodes
        registry.add(new GenericWriter<>(FSROOT_STATE_CONTAINER_ID.child(FapService.class).child(CellConfig.class).child(Lte.class).child(LteRan.class).child(LteRanRf.class), new LteRanRfCustomizer(crudService)));
    }
}
